...
| addons.codeserver.enabled | bool | `false` | Enable Visual Studio code server |
| addons.codeserver.tag | string | latest | Image [tag](https://github.com/linuxserver/docker-code-server/pkgs/container/code-server) |
| addons.codeserver.pullPolicy | string | Always | Image pull policy |
| addons.codeserver.ingress.host | string | `nil` |  |
| addons.codeserver.ingress.path | string | `nil` |  |
| addons.codeserver.ingress.pathType | string | `nil` |  |
| addons.codeserver.persistence | string | `nil` | Persistence has default `config`, use `config` key to override defaults. Define additional volumes to mount as would in main workload or extraContainers |
